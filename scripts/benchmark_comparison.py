...
def main():
    ...
    if args.dataset == "slake":
        ...
        if json_path is None:
            raise FileNotFoundError(
                f"SLAKE test.json not found in '{slake_dir}'.\n"
        ...
        if json_path is None:
            raise FileNotFoundError(
                f"VQA-RAD dataset JSON not found in '{rad_dir}'.\n"
                "Please download VQA-RAD or run 'python run_kaggle_pipeline.py' to prepare datasets."
            )
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # Visual Masking / Saliency Baseline: evaluates model on masked foreground
            masked_images = images * masks
            masked_outputs = vqa_model(masked_images, questions, device)
            masked_logits = masked_outputs["main_class_logits"]
            attn_probs_tensor = torch.softmax(masked_logits, dim=-1)
            attn_pred_classes = torch.argmax(attn_probs_tensor, dim=-1).cpu().numpy()
            attn_preds.extend([idx2ans.get(p, "unknown") for p in attn_pred_classes])
            attn_confidences.extend(attn_probs_tensor.max(dim=-1).values.cpu().numpy())
