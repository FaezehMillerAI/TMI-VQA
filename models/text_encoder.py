...
class ModularTextEncoder(nn.Module):
    def __init__(self, encoder_type: str = "pubmedbert", text_dim: int = 768, max_seq_len: int = 128):
        ...
        # Simple vocabulary for offline LSTM encoding if HuggingFace is offline
        ...
        if "biomedclip" in self.encoder_type:
            try:
                ...
                print(f"Failed to load BiomedCLIP Text Model: {e}. Using offline LSTM encoder.")
        else:
            try:
                ...
                print(f"Failed to load PubMedBERT: {e}. Using offline LSTM encoder.")

    def tokenize_and_encode_fallback(self, texts: List[str], device: torch.device) -> Dict[str, torch.Tensor]:
        """Standalone rule-based tokenizer and bidirectional LSTM sequence encoder when transformers are unavailable."""
        batch_ids = []
        for text in texts:
    ...
    def forward(self, texts: List[str], device: torch.device) -> Dict[str, torch.Tensor]:
        # Fallback to offline LSTM encoder if use_hf is false or not initialized
        if not self.use_hf:
            return self.tokenize_and_encode_fallback(texts, device)
        ...
        try:
            ...
            # Fallback to offline LSTM encoder
            return self.tokenize_and_encode_fallback(texts, device)
