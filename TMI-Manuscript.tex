...
{Faezeh Safari \MakeLowercase{\textit{et al.}}: Causal Contrastive Decoding with Dynamic Question-Conditioned Scaling for Calibrated Medical VQA}
\begin{document}
\title{Causal Contrastive Decoding with Dynamic Question-Conditioned Scaling for Calibrated Medical VQA}
\author{Faezeh Safari, Hang Dong, Aline Villavicencio and Zeyu Fu
\thanks{Department of Computer Science, University of Exeter, England, United Kingdom
 (Faezeh Safari: f.safari@exeter.ac.uk), (Hang Dong: H.Dong2@exeter.ac.uk), (Aline Villavicencio: A.Villavicencio@exeter.ac.uk) and (Zeyu Fu: Z.Fu@exeter.ac.uk).}
\thanks{Code, models, and reproducible pipelines are available at: \url{https://github.com/FaezehMillerAI/TMI-VQA.git}.}}
...
\caption{Overview of the proposed Causal-Interventional Grounding and Counterfactual Inpainting (CI-GCI) framework. (1) Question $Q$ and image $I$ are encoded to locate spatial ROI mask $\mathbf{M}$ via Gaze-Guided ROI Locator (GGRL) cross-attention. (2) The Observational Path computes observational logits $\mathbf{L}_{\text{orig}}$. (3) The Interventional Path applies physical $do$-calculus $do(I \setminus \text{ROI})$ using Generative Counterfactual Inpainting (CFI) to erase lesion evidence into healthy organ background, producing counterfactual logits $\mathbf{L}_{\text{cf}}$. (4) The Causal Contrastive Decoder (CCD) computes calibrated interventional logits $\mathbf{L}_{\text{calib}} = \mathbf{L}_{\text{orig}} + \gamma(Q) \odot \text{ITE}$. (5) The Consistency Verification Head and Selective Abstention Triage Gate route high-confidence predictions to automated output $\hat{A}$ and uncertain cases to human radiologists.}
\label{fig:framework}
\end{figure*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{Manuscript TMI/fig2_calibration_curves.png}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{Manuscript TMI/fig3_multimodal_proof_sheets.png}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{Manuscript TMI/fig4_risk_coverage.png}
