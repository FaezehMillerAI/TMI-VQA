...
from models.causal_decoder import CausalContrastiveDecoder, AnatomyPathologyDisentangledDecoder
...
def run_dataset_inference(
    ...
    if question_type_filter == "closed":
        dataset.data = [item for item in dataset.data if "CLOSED" in str(item.get("answer_type", "")).upper()]
    elif question_type_filter == "open":
        dataset.data = [item for item in dataset.data if "OPEN" in str(item.get("answer_type", "")).upper()]
    # If "all", retain both closed and open questions for comprehensive benchmark evaluation
    ...
    print(f"Loaded {len(dataset)} evaluation samples (filter: '{question_type_filter}').")
    ...
    if model_name == "ci_gci":
        causal_decoder = AnatomyPathologyDisentangledDecoder(
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(pbar):
            ...
            if model_name == "baseline_1" or model_name == "baseline_2":
                ...
                if isinstance(causal_decoder, AnatomyPathologyDisentangledDecoder):
                    causal_out = causal_decoder(
                        orig_logits,
                        cf_logits,
                        question_feats=orig_out.get("q_feat", None),
                        gamma=gamma_tensor
                    )
                else:
            ...
            for i in range(len(questions)):
                ...
                ans_type_raw = str(batch["answer_type"][i]).strip().lower() if "answer_type" in batch else "closed"
                q_type = "closed" if "closed" in ans_type_raw else "open"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--question_type", type=str, default="all", choices=["all", "closed", "open"])
