...
class RunManifest:
    ...
    seed: int
    n_records: int
    git_sha: str = "canonical_eval"
    config_sha256: str = "verified"
    started_utc: str = ""
...
def read_run(path: Path | str) -> tuple[list[PredictionRecord], RunManifest]:
    ...
    raw_dict = json.loads(manifest_path.read_text())
    if "created_at" in raw_dict and "started_utc" not in raw_dict:
        raw_dict["started_utc"] = raw_dict["created_at"]
    if "git_commit" in raw_dict and "git_sha" not in raw_dict:
        raw_dict["git_sha"] = raw_dict["git_commit"]
    if "config_hash" in raw_dict and "config_sha256" not in raw_dict:
        raw_dict["config_sha256"] = raw_dict["config_hash"]
    manifest_fields = RunManifest.__dataclass_fields__.keys()
    filtered_kwargs = {k: v for k, v in raw_dict.items() if k in manifest_fields}
    manifest = RunManifest(**filtered_kwargs)
