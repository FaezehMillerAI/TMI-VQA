...
\begin{abstract}
Medical Visual Question Answering (Med-VQA) models promise to assist clinicians by interpreting multi-modal diagnostic scans through interactive natural language queries. However, existing medical vision-language models remain fundamentally vulnerable to backdoor confounding, language prior shortcuts, and generative hallucinations---frequently outputting plausible diagnostic answers based on text co-occurrences without verifying anatomical evidence in the input scan. Furthermore, severe overconfidence and miscalibration prevent dependable clinical triage. To address these critical barriers, we present \textbf{Causal-Interventional Grounding and Counterfactual Inpainting (CI-GCI)}, a novel causal framework that reformulates Med-VQA from passive observational correlation fitting into an active physical intervention task governed by Pearl\x27s $do$-calculus. CI-GCI integrates: (1) a Gaze-Guided ROI Locator that extracts question-conditioned spatial lesion masks via cross-attention; (2) a Generative Counterfactual Inpainter that physically intervenes on the image by inpainting suspicious pathological tissue into healthy anatomical background, simulating $do(I = I \setminus \text{ROI})$; (3) a Causal Contrastive Decoder that calculates the Individual Treatment Effect scaled by dynamic question semantics $\gamma(Q)$; and (4) a decision-theoretic selective abstention gate. Evaluated across four diverse multi-center benchmarks (VQA-RAD, SLAKE, PathVQA, and Kvasir-VQA-x1), CI-GCI establishes new state-of-the-art accuracy (91.80\% on VQA-RAD, 81.25\% on SLAKE, 69.20\% on PathVQA). Crucially, CI-GCI achieves an 88\% relative reduction in Expected Calibration Error (dropping ECE to 0.0221), cuts visual hallucinations from 38.5\% to 10.8\%, and achieves a 2.4\% clinical error rate at 72.5\% automated coverage, validated by expert radiologist review (Cohen\x27s $\kappa = 0.792$). Full source code, per-sample prediction logs, and reproducible pipelines are publicly available at: \url{https://github.com/FaezehMillerAI/TMI-VQA.git}.
\end{abstract}
